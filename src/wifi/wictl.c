...
static void cmd_reconn(struct top* ctx, int opts, int i, int argc, char** argv)
{
	uc_put_hdr(&ctx->tx, CMD_RECONN);
...
int main(int argc, char** argv)
{
	...
	int mode = opts & (OPT_e | OPT_w | OPT_s | OPT_z | OPT_q | OPT_r);
	...
	if(!mode && i >= argc)
		...
		cmd_reconn(&ctx, opts, i, argc, argv);
	else if(i < argc)
		cmd_wless(&ctx, opts, i, argc, argv);
	else
		fail("bad options", NULL, 0);
